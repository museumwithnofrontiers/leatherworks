...
describe('website smoke test', () => {
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    expect(host.textContent).toContain("Leatherwork")
  ...
  it('colours and names the source-database chip from the manifest projects section', async () => {
    const { app, host } = await mountSite('#/item/5aeb09f1-07de-5c77-8e6d-0fafbac68af4')
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    ...
    const isl = await mountSite('#/item/5aeb09f1-07de-5c77-8e6d-0fafbac68af4')
  ...
  it('renders the related-database and artistic-introduction links from the manifest', async () => {
    const [items] = await loadEntities(['items'])
    const item = items.find((i) => i.id === '5aeb09f1-07de-5c77-8e6d-0fafbac68af4')
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    ...
    const { app, host } = await mountSite('#/item/bba9a6eb-34f8-578b-b58b-ae4a8784d89d')
    ...
    expect(host.textContent).toContain("Aghlabids")
    app.unmount()
  }, 60000)
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    expect(host.textContent).toContain("Morocco")
    expect(host.textContent).toContain('Ministry of Culture, Directorate for Cultural Heritage')
  ...
  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/8ea7243d-fe8e-5011-a9ca-75ebd26660ac')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('Ministry of Culture, Directorate for Cultural Heritage'), { timeout: 20000 })
    expect(host.textContent).toContain("Rabat")
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/8ea7243d-fe8e-5011-a9ca-75ebd26660ac/objects')
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['leatherworks', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
